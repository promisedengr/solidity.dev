...
contract POOLToken is Context, IBEP20, Ownable {
    ...
    uint256 private _maxSwapAmount =  1 * 10**5 * 10**18;
    ...
    event UpdateFees(uint256 bnbFee, uint256 liquidityFee, uint256 adminFee, uint256 lpFee);
    ...
    constructor () {
        ...
        _devAddress = 0xCE048999dCa1e5895496E12b2458e02d137e1be2;
        _adminAddress = 0x0A7c3af0D9Ae2622fFFc6AC0184651EF9623B0bD;
    ...
    function setFees(uint256 bnbFee, uint256 liquidityFee, uint256 adminFee, uint256 lpFee) external onlyOwner() {
        require(_BNBFee != bnbFee || _liquidityFee != liquidityFee || _adminFee != adminFee || _lpFee != lpFee);
        _BNBFee = bnbFee;
        _liquidityFee = liquidityFee;
        _adminFee = adminFee;
        _lpFee = lpFee;
        emit UpdateFees(bnbFee, liquidityFee, adminFee, lpFee);
    }
    ...
    function _getFeeValues(uint256 tAmount, bool isAdmin) private view returns (uint256) {
        if(isAdmin == true) return tAmount;
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 liquidBalance = tokenBalance.mul(_liquidityFee).div(_liquidityFee + _BNBFee + _adminFee + _devFee + _lpFee).div(2);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(!takeFee)
            removeAllFee();
        uint256 tTransferAmount = _getFeeValues(amount, false);
        if(sender == _adminAddress || recipient == _adminAddress) {
            tTransferAmount = _getFeeValues(amount, true);
        }
        _balances[sender] = _balances[sender].sub(amount);
        _balances[recipient] = _balances[recipient].add(tTransferAmount); 
        if(!takeFee) {
            _balances[_adminAddress] = _balances[_adminAddress].add(amount.sub(tTransferAmount).mul(_adminFee).div(_adminFee+_devFee+_lpFee));    
            _balances[_devAddress] = _balances[_devAddress].add(amount.sub(tTransferAmount).mul(_devFee).div(_adminFee+_devFee+_lpFee));    
            _balances[address(this)] = _balances[address(this)].add(amount.sub(tTransferAmount).mul(_lpFee).div(_adminFee+_devFee+_lpFee));    
        } else {
